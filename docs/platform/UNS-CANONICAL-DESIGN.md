...
| D‑U27 | Reserved-class guard binds the **effective** root, not the raw `includeRoot` flag | ✅ **resolved 2026-07-03** (slice-1f finding): each facade binds the guard's `includeRoot` to `includeRoot && hier.size() ≥ 2` — the same effective-root rule topic-building uses (D‑U25). Otherwise a warned single-level+`includeRoot` misconfig makes the guard check class position 5 while the component's own topics are rootless, false-positive-rejecting a legit `app`/`evt`/`data` channel whose first token is a reserved word. **Applies to all 4 langs.** | High | Easy | no |
