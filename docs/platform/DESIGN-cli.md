...
| D-CLI-24 | **A definition is one shared `topology` + per-platform `profiles`, not one flat single-platform document** (`DESIGN-deployment-profiles.md`). The topology carries each component's functional half (`layer`, `messaging`, `files`, `catalogKey`); a profile carries the delivery half (`configSource`, `artifact`/`image`, `launch`, node scaffolding) and an optional `deploys` selection. `effective(profile)` merges them into the flat form the renderers consume, so `--target <family>` selects a profile and the renderers are unchanged. The flat `targetStandard`+`nodes` shape is removed (no backward compat). | Define the plant once, deploy it anywhere: Dallas existed as N divergent per-platform definitions (`dallas` HOST full, `dallas-gg` a 3-component subset with duplicated layers) that had already drifted. One topology proves HOST, Greengrass, and Kubernetes from a single source; the split keeps functional config single-sourced while platform delivery legitimately differs. |
| D-CLI-25 | **A Kubernetes component's artifact is its `image`** (S-6 accepts an image *or* a version/source), and the Kubernetes renderer emits per-component ServiceAccount + ConfigMap + Deployment + Service with the `CONFIGMAP` source mounted whole-volume (no checksum annotation, to keep hot-reload), Downward-API identity, non-root/read-only security, and `nodeSelector` placement. | A Deployment cannot be built without an image; "what runs" on Kubernetes is the image, as version+digest is on Greengrass. The manifest shape follows the proven `test-infra/k8s` chart and interop manifests, so the renderer *replaces* the hand-maintained chart (REVIEW #11's demand-gate). |
