...
| [DESIGN-config-numeric-canonicalization.md](DESIGN-config-numeric-canonicalization.md) | **Numeric canonicalization at the config intake boundary:** why a store that round-trips JSON numbers as doubles (the Greengrass Nucleus) killed strictly-typed components at startup, the pure/idempotent pass that fixes it once for every source and every language, the exact accept/reject semantics table, and the decisions register (D‑NC1…D‑NC5) including the removal of silent truncation and the `tags` scope call. |
