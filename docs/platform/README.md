# Platform model (platform × transport) for edgecommons — Executive Summary & index
...
> package for **re-architecting edgecommons' runtime model into two orthogonal axes — `platform ×
...
Kubernetes already *works* under STANDALONE — you can run a edgecommons component in a pod today. But
...
edgecommons already constructs every AWS SDK client **without explicit credentials**, relying on the
...
**not edgecommons code** — zero per-provider branches in any language. For the edge/on-prem target:
...
- **Private / air-gapped: IAM Roles Anywhere (X.509)** — no public API-server exposure; aligns with
  edgecommons' existing IoT X.509 heritage. This is the primary path for disconnected fleets.
...
- **Do not build a custom operator now.** Nothing in edgecommons is a stateful Day-2 problem that
  needs encoded operational knowledge; an operator would also be the first Go binary in an otherwise
  Java/Python/Rust/TS repo. An `EdgeCommonsComponent` CRD **sketch** is included for completeness, with
...
| [Hierarchical config design](hierarchical-config-design/index.html) | Interactive design for the arbitrary-depth enterprise/site/building/zone/line/device/component lineage model while keeping one validated effective config at runtime. |
...
| [DESIGN-operator.md](DESIGN-operator.md) | The optional `EdgeCommonsComponent` CRD sketch and the explicit "don't build it yet / build it when…" recommendation. |
| [DESIGN-uns.md](DESIGN-uns.md) | The **Unified Namespace**: topic grammar, message classes, configurable hierarchy + top-level identity (distributed via `../HIERARCHICAL_CONFIG.md`), the `messaging()`/`uns()`/facade API, streaming enrichment, and the `uns-bridge` site-bus realization. Concretizes [DESIGN-channels.md](DESIGN-channels.md); consumed by the `edgecommons/edge-console` component. |
...
| [DESIGN-binary-messaging.md](DESIGN-binary-messaging.md) | **Binary Messaging proposal:** corrects the current top-level-only binary marker into two useful capabilities: byte arrays as typed values inside normal JSON telemetry bodies, and true MQTT/Greengrass opaque binary frames with compact EdgeCommons headers. Includes UNS, telemetry-processor, uns-bridge, streaming, interop, and Greengrass validation impacts. |
| [DESIGN-cli.md](DESIGN-cli.md) | **The `edgecommons` CLI (Rust, greenfield) — PROPOSED:** the noun–verb command surface, the language × kind template model (manifest v2 + platform packs), the shared validation engine (config schema + semantic rules + parsed artifact lint), `registry`/`doctor`, and — at contract level — the `deployment` verbs, the hexagonal kernel, and the five ports that make the binary the Deployment Studio kernel (RM-012). Carries the **defect register** that supersedes the Python CLI's test suite as the behavioral oracle, plus the decisions register (D‑CLI‑1…D‑CLI‑9). |
| [ANALYSIS-http-transport.md](ANALYSIS-http-transport.md) | **HTTP(S) as a component-to-component transport for HOST/KUBERNETES — ANALYSIS ONLY, nothing accepted or built.** Splits EdgeCommons traffic into its *addressed* half (request/reply, command inbox, config push — native to HTTP) and its *fan-out* half (the UNS publish classes — not native), shows why HTTP cannot remove the broker from pub/sub but only change who writes it, and inventories what earlier design choices (no retain, no LWT, clean session, protobuf envelope, one transport-injection site, an embedded HTTP server already in all four languages) already buy. Four options — addressed-only surface, full transport + hub, sidecar, gRPC — with a recommendation and the open questions that must be decided first. |
