# EdgeCommons — Rust Port: Design & Implementation Plan

Status: **Complete — all phases delivered and validated on-device** (HOST platform, cross-language parity, and Greengrass IPC incl. `GG_CONFIG`/`SHADOW`/`CONFIG_COMPONENT` and the real-time device-shadow round-trip, validated against a live Greengrass core, non-root) · Target: feature parity with `edgecommons-java-lib`
...
Produce a Rust library, crate name **`edgecommons`** (published as `edgecommons` to match the Python package), that lets authors build AWS IoT Greengrass v2 components in Rust while writing only business logic — exactly the value proposition of the Java/Python libraries. It bundles the cross-cutting concerns behind service interfaces: configuration, messaging, metrics, heartbeat, and logging.
...
| IPC operation | edgecommons subsystem | Covered |
|---|---|---|
...
| `GetSecretValue` | *(not used by edgecommons)* | n/a |
...
edgecommons/
  ...
  src/
    lib.rs              # public API: EdgeCommons, EdgeCommonsBuilder, prelude
    error.rs            # EdgeCommonsError (thiserror), Result alias
...
The component's `main` is a `#[tokio::main]` that builds `EdgeCommons`, retrieves services, and runs business logic. Long-running tasks (heartbeat ticks, CloudWatch batch flush, config watchers) are `tokio::task`s, not OS threads — a panic in one is isolated and observable, eliminating the Java "Timer thread dies → feature silently stops forever" failure mode.

---

## 8. Error handling strategy

- Library returns `Result<T, EdgeCommonsError>`. **No `process::exit` anywhere in the library** (the Java code had 18 such call sites). `EdgeCommonsError` is a `thiserror` enum with variants per subsystem (`Config`, `Messaging`, `Metrics`, `Ipc`, `Validation`, `Cli`).
...
No runtime type-keyed registry (unidiomatic in Rust). Instead, `EdgeCommons` holds the wired services as trait objects, exposed by typed accessors:
...
The trait objects are the testable seam: inject fakes in tests (replacing the Java `MockMessagingService`/`TestableEdgeCommons` harness). RAII handles shutdown — dropping `EdgeCommons` stops the heartbeat task, closes MQTT/IPC clients, and cancels watchers (fixes the Java "no `close()` anywhere" leak class, review H3).
...
Builders (`EdgeCommonsBuilder`, `MessageBuilder`, `MetricBuilder`) mirror the Java fluent API so the mental model transfers.
...
The embedded JSON schema is the synced copy of the canonical `schema/edgecommons-config-schema.json`
...
- `error.rs` (`EdgeCommonsError`); `cli.rs` (clap, full `-c`/`--platform`/`--transport`/`-t` contract incl. IPC-only-on-GREENGRASS validation and full-string `-t`).
...
- ✅ Wired into `EdgeCommonsBuilder::build` behind the `greengrass` cargo feature.
...
- ✅ Config snapshot edge cases, **multi-instance** (`instance_ids()`/`instance(id)`, verified end-to-end through `EdgeCommons`), logging runtime reconfiguration parity.
...
4. **Crate/publishing:** crate name `edgecommons`, published as `edgecommons`; registry (crates.io vs internal GitLab).
...
- Internal: `edgecommons-java-lib` code review (this engagement); workspace `CLAUDE.md`.
