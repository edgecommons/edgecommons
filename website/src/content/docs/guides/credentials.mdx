...
In hierarchical-config deployments, shared provider settings such as central sync `from` mappings
can live in an inherited hierarchy layer. The vault itself remains per component unless you
deliberately configure a served credentials manager; inheriting settings does not create one shared
on-disk vault file.
