...
In split-config deployments, shared provider settings such as central sync `from` mappings can live
in the shared base layer. The vault itself remains per component unless you deliberately configure a
served credentials manager; inheriting settings does not create one shared on-disk vault file.

<Aside type="caution" title="cacheTtlSecs is a no-op">
The schema accepts `vault.cacheTtlSecs` (default `300`), but **no SDK reads it** — Java,
...
```toml
[dependencies]
edgecommons = { version = "*", features = ["credentials"] }
...
| `env` | A base64 KEK from an env var / mounted Secret — the offline software KEK, default on KUBERNETES | `envVar` (default `EDGECOMMONS_VAULT_KEK`) |
...
```json
{
  "credentials": {
    "vault": {
      "keyProvider": { "type": "env", "envVar": "EDGECOMMONS_VAULT_KEK" }
    }
  }
}
```
...
`EDGECOMMONS_VAULT_KEK`), tolerates a trailing newline, and errors on unset/empty/bad-base64/wrong
...
$env:EDGECOMMONS_VAULT_KEK = [Convert]::ToBase64String($bytes)
```
    ...
    "streams": [
      {
        ...
        "buffer": { "type": "disk", "path": "/var/lib/edgecommons/streams/events" }
      }
    ]
  }
}
...
|----------|-----------------------|
| Java | `com.mbreissi.edgecommons.credentials.audit` |
| Python | `edgecommons.credentials.audit` |
| Rust | `edgecommons::credentials::audit` (tracing target) |
...
- The `credentials` section of the [config schema](/reference/configuration-schema/) — every field,
