...
Any config element can reference a vault secret instead of embedding it, so messaging/streaming
...
The SDK resolves these against the credential service. In Java and TypeScript the resolver returns a
new (deep-copied) structure and never mutates the input; in Python and Rust it mutates the passed
structure in place.

{/* NEEDS REVIEW: $secret resolution is wired automatically into the messaging/streaming config
load path vs. invoked manually by the component — confirm the exact integration point before
documenting a call site here. */}
