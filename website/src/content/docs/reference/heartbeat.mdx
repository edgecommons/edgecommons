...
<Aside type="danger" title="heartbeat.targets is not accepted">
The heartbeat has no `targets[]` array and no per-target topic/destination overrides — the
keepalive topic is always the component's UNS state topic, and the measures route through the
metric subsystem. A config carrying `heartbeat.targets` **fails validation at startup**. See
the [guide's configuration section](/guides/heartbeat/#configuration).
</Aside>
