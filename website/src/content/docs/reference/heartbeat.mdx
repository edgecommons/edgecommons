...
The EdgeCommons runtime constructs the heartbeat during initialization and tears it down on close.
**No SDK exposes a public accessor** — there is no `getHeartbeat()` / `heartbeat()` /
`get_heartbeat()` on the EdgeCommons object in any language. Component code drives the heartbeat
...
`com.mbreissi.edgecommons.heartbeat.Heartbeat` implements `ConfigurationChangeListener`. Its
...
`edgecommons::heartbeat::Heartbeat` wraps the background tokio task. Its `start` is **crate-private**
...
`Heartbeat` is exported from `edgecommons`. Its constructor is private — the runtime uses the static
`start` factory.
...
`start` is crate-private — only `EdgeCommonsBuilder::build()` calls it, handing the heartbeat the
...
```rust
use edgecommons::heartbeat::HeartbeatMonitor;
use edgecommons::config::model::Measures;
...
`edgecommons.config.heartbeat_config.HeartbeatConfiguration`:
