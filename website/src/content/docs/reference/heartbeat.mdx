...
version `1.0`, body `{"status":"RUNNING","uptimeSecs":n}`) through the library's privileged
...
through the [`heartbeat` config section](#the-heartbeat-config-shape), not through these APIs. And
