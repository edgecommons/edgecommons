...
```python
# module ggcommons.credentials.config
...
```rust
// module ggcommons::credentials (re-exported from config.rs); requires the `credentials` feature
...
```typescript
// module ggcommons/credentials/config — ASYNC (returns a Promise)
...
A minimal read, guarding for the absent service first. (Verified against each SDK's credentials test
suite and dev guide.)
