...
In split-config deployments, the shared base layer may provide common credentials provider or
central-sync settings. That inheritance does not create a shared on-disk vault; each component still
owns its configured vault unless it is explicitly pointed at a served credentials manager.
