...
| SDK | Module / package |
|---|---|
| Java | `com.mbreissi.edgecommons.platform` (the resolver, enums, and profile types); `com.mbreissi.edgecommons.EdgeCommons` (the parser entry point `processArgs`). |
| Python | `edgecommons.platform` — submodules `platform` (the `Platform` enum), `transport` (the `Transport` enum), and `resolver` (everything else). |
| Rust | `edgecommons::platform` (enums, profile types, resolver fns); `edgecommons::cli` (the `parse_from` entry point). |
| TypeScript | the `@edgecommons/edgecommons` package root re-exports from `platform.ts` and `cli.ts`. |
...
```python
# module: edgecommons.platform.resolver
...
```rust
// module: edgecommons::platform
// Returns EdgeCommonsError::Cli on the IPC lock or an unprofiled platform.
...
```typescript
// module: edgecommons (re-exported from platform.ts)
...
// throws EdgeCommonsError (kind "Cli") on the IPC lock
...
`validate` is exported from the `edgecommons` package root as **`validatePlatformTransport`** (to avoid
...
```java
import com.mbreissi.edgecommons.platform.*;
...
from edgecommons.platform.resolver import ResolverInputs, resolve_profile
...
use edgecommons::platform::{resolve_profile, Platform, ResolverInputs};
...
```typescript
import { resolveProfile, Platform, type ResolverInputs } from "@edgecommons/edgecommons";
...
| `ENV_K8S_THING_NAME` | `EDGECOMMONS_THING_NAME` | Kubernetes Downward-API identity (primary k8s tier). |
...
`/etc/edgecommons`, key `config.json`.
...
- **`validate`** — rejects `transport == IPC && platform != GREENGRASS` (the IPC lock). Java raises
  `IllegalArgumentException`, Python raises `ValueError`, Rust returns `EdgeCommonsError::Cli`, TypeScript
  throws a `EdgeCommonsError` of kind `Cli`.
