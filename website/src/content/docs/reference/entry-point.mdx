...
```typescript
import { GGCommonsBuilder, GGCommons } from "@edgecommons/ggcommons";
...
GgInstance        instance(String id);  // instance-scoped handle (validated token; cached per id)
...
def instance(self, instance_id: str) -> GgInstance # instance-scoped handle (validated; cached per id)
...
fn instance(&self, instance_id: &str) -> Result<GgInstance>;          // Err on an invalid token
...
instance(instanceId: string): GgInstance;          // instance-scoped handle (validated; cached per id)
