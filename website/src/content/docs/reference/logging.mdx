...
This page is the API reference for GGCommons logging: the logger type in each SDK, how you obtain
...
`logger` is the process-wide root `Logger` (named `"ggcommons"`). `getLogger(name)` returns a named,
...
`com.mbreissi.ggcommons.logging.Logger` is an interface. Each of `trace`/`debug`/`info`/`warn`/`error`
...
{/* NEEDS REVIEW: the Logger interface has class-level Javadoc but NO per-method Javadoc in source (libs/java/.../logging/Logger.java); the overload/predicate descriptions above are derived from the signatures. Confirm the SLF4J-style {} semantics hold for the Log4j2 and JUL adapters too. */}
...
These are stdlib semantics; GGCommons only configures the root logger, level, format, and sinks.
...
export class Logger {
  ...
  constructor(name?: string);                       // defaults to "ggcommons"
...
{/* NEEDS REVIEW: Logger.debug/info/warn/error have NO JSDoc in source (libs/ts/src/logging.ts:440-451); the signatures and stream-routing note are read directly from source. The Level enum and RotatingFileWriter are internal/un-exported — components set levels via config (logging.level / logging.loggers), not via setLevel. */}
...
{/* NEEDS REVIEW: configure_logging() installs a bare logging.StreamHandler() whose default stream is sys.stderr (for both text and JSON modes). Confirm on a real run whether Python's JSON lines land on stdout vs stderr before documenting "stdout-JSON" for Python specifically. */}
...
```typescript
import { initLogging, reconfigureLogging, LoggingReconfigurer } from "@edgecommons/ggcommons";
