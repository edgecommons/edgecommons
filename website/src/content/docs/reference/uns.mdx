...
publish them. Before this facade slice, that meant hand-minting the topic with `uns()` and
hand-assembling the body, and it had already drifted between components (one adapter's `evt` channel
carried a severity segment, another's didn't; each re-invented its own `SouthboundSignalUpdate`
quality default). `data()`/`events()`/`app()` are the **sanctioned publish path**: they still go
through the ordinary, guarded `messaging().publish(...)` — publishing `data`/`evt`/`app` is not a
privileged operation, so raw `uns()` + `messaging()` publishing (as shown above) remains fully
...
```java
// com.mbreissi.ggcommons.facades.DataFacade — via gg.getData() / gg.instance(id).data()
...
```python
# ggcommons.facades.data_facade.DataFacade — via gg.data() / gg.instance(id).data()
class DataFacade:
...
`evt/{severity}/{sanitize(type)}` — so the topic and the body can never disagree, the exact drift
that motivated this facade (one adapter's channel carried severity, another's didn't; this makes it
structural). `timestamp` defaults to now; `type` is the only required field (it is a channel token).
...
```java
// com.mbreissi.ggcommons.facades.EventsFacade — via gg.getEvents() / gg.instance(id).events()
...
```python
# ggcommons.facades.events_facade.EventsFacade — via gg.events() / gg.instance(id).events()
class EventsFacade:
...
```java
// com.mbreissi.ggcommons.facades.AppFacade — via gg.getApp() / gg.instance(id).app()
...
```python
# ggcommons.facades.app_facade.AppFacade — via gg.app() / gg.instance(id).app()
class AppFacade:
...
`header.name` must equal. It ships in **all four languages** (not Java only), pinned by
