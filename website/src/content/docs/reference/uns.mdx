...
`hierarchy`, `identity`, and `topic` are read from the merged effective config. In split-config
deployments, a shared base layer can define the device/site identity once and every component then
publishes with the same resolved identity contract.
...
- **`northbound`** — publishes straight to AWS IoT Core (`messaging().publishNorthbound`); a
...
Every publish path that takes a client-chosen topic — `publish`, `publishRaw`, `publishNorthbound`,
