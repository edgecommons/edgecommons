...
- The default `cache.path` when unset is the relative `"param-cache"`. The accessor path substitutes
  template variables such as `{ComponentFullName}` and `{ThingName}` before calling the factory; the
  raw factory does no templating.
