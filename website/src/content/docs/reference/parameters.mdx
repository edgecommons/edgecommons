...
In hierarchical-config deployments, an inherited hierarchy layer may define common
`parameters.source` and cache defaults. Each component still owns its own `ParameterService`, cache
lifecycle, and sync set.
...
Fetch the `ParameterService` from your `EdgeCommons` instance. It is `null` / `None` / `undefined`
