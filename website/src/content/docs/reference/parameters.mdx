...
In split-config deployments, the shared base layer may define common `parameters.source` and cache
defaults. Each component still owns its own `ParameterService`, cache lifecycle, and sync set.
