...
$canon = Join-Path $root 'schema\ggcommons-config-schema.json'

# canonical -> destination copies (TS uses a different filename)
$targets = @(
  'libs\rust\resources\ggcommons-config-schema.json',
  'libs\ts\src\config\schema.json',
  'libs\python\ggcommons\resources\ggcommons-config-schema.json',
  'libs\java\src\main\resources\ggcommons-config-schema.json',
  'libs\java\doc\ggcommons-config-schema.json'
...
if ($Check -and $drift) {
  Write-Host ""
  Write-Host "Config schema copies are out of sync with schema/ggcommons-config-schema.json." -ForegroundColor Red
