...
- **Two axes:** language × **kind** (`service`, `protocol-adapter`, `processor`, `sink`). Today the matrix is complete — all four languages × all four kinds (`{java,python,rust,typescript}` × `{service,protocol-adapter,processor,sink}`, 16 templates) — run `edgecommons template list`. Each archetype template teaches its shape (the adapter's `DeviceSession` seam + SouthboundSignalUpdate publish; the processor's `Processor` stage trait + self-echo guard; the sink's `Destination` seam + verify-before-confirm + jittered retry), and each ships a `config.schema.json` and passing tests.
