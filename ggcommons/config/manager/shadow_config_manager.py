...
class ShadowConfigManager(ConfigManager, ABC):
    ...
    _DEFAULT_CONFIGURATION = { "logging": {}, "source": {}, "heartbeat": {}, "component": { "global": {}, "instances": [] }}
    ...
    def __init__(self, component_name: str, shadow_name: str):
        ...
        self._shadow_topic_prefix = ShadowConfigManager._SHADOW_TOPIC_TEMPLATE.format(self.get_thing_name(), shadow_name)
    ...
    def _subscribe_to_shadow_topics(self):
        ...
        try:
            shadow_update_delta_topic = ShadowConfigManager._ALL_SHADOW_TOPIC_TEMPLATE.format(self.get_thing_name(), self._shadow_name)
            _, operation = self._ipc_client.subscribe_to_topic(topic=shadow_update_delta_topic,
    ...
    def _on_shadow_event(self, event: SubscriptionResponseMessage) -> None:
        ...
        logger.debug(f"Received shadow message for shadow action '{action}' result '{result}'. Payload: {payload_str}")

        if action == "get" and result == "rejected":
            logger.warning(f"Named shadow document {self._shadow_name} does not exist. Creating default configuration.")
            self._report_updated_configuration(ShadowConfigManager._DEFAULT_CONFIGURATION)
    ...
    def _report_updated_configuration(self, config: dict) -> None:
        shadow_doc = {
            "state": {
                "reported": {
                    "ComponentConfig": json.dumps(config, indent=None, separators=(',',':'))
                }
            }
        }
        logger.debug(f"Reporting updated configuration to named shadow document '{self._shadow_name}': {shadow_doc}")
        try:
            self._ipc_client.update_thing_shadow(thing_name=self.get_thing_name(), shadow_name=self._shadow_name,
    ...
    def _get_configuration(self):
        try:
            response = self._ipc_client.get_thing_shadow(thing_name=self.get_thing_name(), shadow_name=self._shadow_name)
            if response.payload is not None and len(response.payload) > 0:
                ...
                logger.warning(f"Named shadow document '{self._shadow_name}' does not exist or is empty")
            return None
        except Exception as e:
