...
| `messaging` | Publish EMF in a `Metric` message envelope on the library-owned UNS metric topic `ecv1[/{site}]/{device}/{component}/metric/{metricName}` (via the reserved-publish seam; the legacy `targetConfig.topic` override is removed) | `targetConfig.destination`: `ipc`/`local` or `northbound` |
...
> `ecv1[/{site}]/{device}/{component}/metric/{metricName}`, and `cloudwatchcomponent` always
