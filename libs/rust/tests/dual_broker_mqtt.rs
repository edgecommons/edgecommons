...
//! provider connected to BOTH a local broker AND northbound MQTT at the same time, using
//! both transports.
//!
//! To run without real cloud infrastructure, the `northbound` endpoint is pointed at the SAME
//! shared EMQX as `local` but over the mutual-TLS listener (:8883), so the real
...
//!
//! Gated: requires `EDGECOMMONS_IT_MQTT=1` plus the mutual-TLS material for northbound
//! — `EDGECOMMONS_IT_MQTT_CA`, `EDGECOMMONS_IT_MQTT_CERT`, `EDGECOMMONS_IT_MQTT_KEY`
//! (this test chooses TLS for the northbound broker). No-ops otherwise.
//!
//! ```bash
...
fn creds_or_skip() -> Option<(String, String, String)> {
    ...
    match (ca, cert, key) {
        ...
        _ => {
            eprintln!(
                "skipping dual-broker test (northbound TLS needs: set \
...
async fn connect_dual() -> Option<Arc<DefaultMessagingService>> {
    ...
    let mc: MessagingConfig = serde_json::from_value(json!({
        "messaging": {
            ...
            "northbound": {
                "host": host,
                "port": tls_port,
                "clientId": format!("it-dual-northbound-{}", Uuid::new_v4()),
    ...
    assert!(
        ...
        "northbound must be configured"
    );

    let provider = Arc::new(
        MqttProvider::connect(&mc)
            .await
            .expect("dual-broker connect"),
    );
...
fn msg(name: &str, payload: serde_json::Value) -> edgecommons::messaging::message::Message {
    MessageBuilder::new(name, "1.0")
        .payload(payload)
        .tag("origin", serde_json::json!("dual-thing"))
        .build()
}

#[tokio::test]
async fn both_transports_deliver_simultaneously() {
    let Some(svc) = connect_dual().await else {
        return;
    };
    ...
    svc.subscribe(
        ...
        message_handler(move |_t, m| {
            let lh = lh.clone();
            async move {
                *lh.lock().unwrap() = Some(m.body.clone());
            }
        }),
        16,
        1,
    ...
    svc.subscribe_northbound(
        ...
        message_handler(move |_t, m| {
            let ih = ih.clone();
            async move {
                *ih.lock().unwrap() = Some(m.body.clone());
            }
        }),
        Qos::AtLeastOnce,
        16,
    ...
    svc.publish_northbound(
        &iot_topic,
        &msg("IotMsg", json!({ "via": "iot" })),
        Qos::AtLeastOnce,
    )
    .await
    ...
    let local_body = local_rx
        .lock()
        .unwrap()
        .clone()
        .expect("local message delivered");
    let iot_body = iot_rx
        .lock()
        .unwrap()
        .clone()
        .expect("iot core message delivered");
    assert_eq!(local_body["via"], "local");
    assert_eq!(iot_body["via"], "iot");

    svc.unsubscribe(&local_topic).await.ok();
    svc.unsubscribe_northbound(&iot_topic).await.ok();
}
...
async fn request_reply_on_both_transports() {
    ...
    svc.subscribe(
        ...
        message_handler(move |_t, request| {
            ...
            async move {
                let _ = responder
                    .reply(&request, msg("LReply", json!({ "answer": "local" })))
                    .await;
            }
        }),
        16,
    ...
    let local_future = svc
        .request(&local_req, local_request)
        .await
    ...
    svc.subscribe_northbound(
        ...
        message_handler(move |_t, request| {
            ...
            async move {
                let _ = responder2
                    .reply_northbound(&request, msg("IReply", json!({ "answer": "iot" })))
                    .await;
            }
        }),
        Qos::AtLeastOnce,
    ...
    let iot_future = svc
        .request_northbound(&iot_req, iot_request)
        .await
        .expect("iot request");
    ...
    svc.unsubscribe_northbound(&iot_req).await.ok();
}
