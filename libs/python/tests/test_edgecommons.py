"""
Integration tests for EdgeCommons.
"""
...
def edgecommons_instance():
    """Create EdgeCommons instance for testing."""
    try:
        edgecommons = EdgeCommonsBuilder.create("edgecommons_python") \
            .with_args([
                '-c', 'FILE', 'c:/Users/breis/source/edgecommons/edgecommons-python-lib/tests/test-config.json',
                '--platform', 'HOST', '--transport', 'MQTT', 'c:/Users/breis/source/edgecommons/edgecommons-python-lib/tests/test-messaging-config.json',
                '-t', 'edgecommons-test-2'
            ]) \
            .build()
        ...
        pytest.skip(f"Failed to initialize EdgeCommons: {e}")
    finally:
...
@pytest.fixture
def messaging_service(edgecommons_instance):
    """Get the messaging handle from the EdgeCommons instance."""
    return edgecommons_instance.get_messaging()


@pytest.fixture
def config_service(edgecommons_instance):
    """Get the configuration manager from the EdgeCommons instance."""
    return edgecommons_instance.get_config_manager()
...
def test_initialization(edgecommons_instance, messaging_service, config_service):
    """Test enhanced initialization with builder pattern."""
    assert edgecommons_instance is not None
