...
Reuses the ``edgestreamlog`` durable log + at-least-once export engine via a **host-callback sink**
...
class CloudWatchDrain:
    ...
    ``edgestreamlog`` callback. Thread-safe: ``drain_batch`` runs on the native export engine thread.
    """
