...
_DEFAULT_PATH_TEMPLATE = "/var/lib/edgecommons/metrics/{ComponentName}/cw"
...
class CloudWatch(MetricTarget):
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        # Durable store-and-forward path (edgestreamlog): selected when targetConfig.cloudwatch.buffer
        ...
        if str(buffer.get("type", "durable")).lower() == "durable":
            ...
            if not native_available():
                ...
                raise RuntimeError(
                    "Durable CloudWatch buffer (the default) requires the edgestreamlog native core, "
                    "which is not installed for this platform. Install the 'edgestreamlog-native' "
