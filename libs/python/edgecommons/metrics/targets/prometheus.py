...
* gauge **name** = :func:`_sanitize_metric_name` of ``lowercase("{namespace}_{measureName}")`` —
  ...
  (Prometheus metric-name rules); ``namespace`` defaults to ``edgecommons``.
...
class _MetricsRequestHandler(BaseHTTPRequestHandler):
    ...
    server_version = "edgecommons-prometheus"
    sys_version = ""
...
class Prometheus(MetricTarget):
    ...
    def _start_server(self) -> None:
        ...
        self._thread = Thread(
            target=self._httpd.serve_forever, name="edgecommons-prometheus", daemon=True
        )
        self._thread.start()
        self.logger.info(
