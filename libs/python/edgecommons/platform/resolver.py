...
**Phase 0:** :attr:`~edgecommons.platform.platform.Platform.GREENGRASS` and
:attr:`~edgecommons.platform.platform.Platform.HOST` both default their config
source to ``GG_CONFIG`` (a faithful re-expression of today's behavior — HOST
does *not* flip to ``FILE`` until Phase 1).

**Phase 1a:** :attr:`~edgecommons.platform.platform.Platform.KUBERNETES` now has a
...
``log`` — applied by :class:`~edgecommons.metrics.metric_emitter.MetricEmitter` when it selects
...
from edgecommons.platform.transport import Transport
...
#: Kubernetes Downward-API identity (Phase 1b): the chart maps the ``edgecommons.io/thing-name`` pod
#: annotation (or an explicit value) into this env var. Highest of the KUBERNETES identity tier.
ENV_K8S_THING_NAME = "EDGECOMMONS_THING_NAME"
#: Kubernetes Downward-API pod name (Phase 1b): ``metadata.name`` via a Downward-API ``fieldRef``.
#: The fallback identity on KUBERNETES when ``EDGECOMMONS_THING_NAME`` is absent.
...
def profile_health_enabled(platform: Optional[Platform]) -> bool:
    ...
    ``EdgeCommons._init_health``). It is a pure lookup (no I/O, no ``ConfigManager`` dependency), so the
...
def profile_metric_target(platform: Optional[Platform]) -> Optional[str]:
    ...
    :class:`~edgecommons.metrics.metric_emitter.MetricEmitter` when it selects the target. It is a pure
...
def profile_credentials_key_provider(platform: Optional[Platform]) -> Optional[str]:
    ...
    ``file`` — consulted at the credentials init site (``EdgeCommons._init_credentials``) when a
...
def resolve_identity(
    ...
    2. **if** ``platform == KUBERNETES``: the Downward-API env vars, in order —
       :data:`ENV_K8S_THING_NAME` (``EDGECOMMONS_THING_NAME``, the
       ``edgecommons.io/thing-name`` pod annotation mapped by the chart), then
    ...
    if thing is not None:
        return thing
    # KUBERNETES Downward-API identity tier (FR-RT-7): EDGECOMMONS_THING_NAME, then POD_NAME.
