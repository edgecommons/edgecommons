...
optional per-call :class:`~edgecommons.facades.channel.Channel` override).
:meth:`~edgecommons.facades.data_facade.DataFacade.build_body` applies the defaulting
rules (quality -> ``GOOD``, ``serverTs`` -> now, the ``samples`` wrapper) and produces
the wire body.

Obtain a builder from :meth:`~edgecommons.facades.data_facade.DataFacade.signal` and
terminate with :meth:`SignalUpdateBuilder.publish` (or :meth:`SignalUpdateBuilder.build`
for the :meth:`~edgecommons.facades.data_facade.DataFacade.publish_update` form). The
...
(``com.mbreissi.edgecommons.facades.SignalUpdate``); Python collapses the Java ``Sample.of``
...
class SignalUpdateBuilder:
    ...
    :param facade: the originating :class:`~edgecommons.facades.data_facade.DataFacade`
        (``None`` for a **detached** builder -- terminate with :meth:`build` and pass the
        result to :meth:`~edgecommons.facades.data_facade.DataFacade.publish_update`)
    """
    ...
    def via(self, channel: Channel) -> "SignalUpdateBuilder":
        """Sets a per-call :class:`~edgecommons.facades.channel.Channel` override
    ...
    def build(self) -> SignalUpdate:
        ...
        :meth:`~edgecommons.facades.data_facade.DataFacade.publish_update` form)."""
        return SignalUpdate(
            device=self._device,
