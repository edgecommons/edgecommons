"""
High-level Python API for the ``edgestreamlog`` telemetry-streaming core.

Wraps the ``edgestreamlog_native`` PyO3 module (a native extension built from the shared Rust core,
...
# runtime dependency: importing edgecommons.streaming must not hard-fail a component that doesn't
...
try:
    import edgestreamlog_native as _native
...
def native_available() -> bool:
    """``True`` if the ``edgestreamlog-native`` wheel is importable — i.e. telemetry streaming and the
...
def _require_native():
    ...
    if _native is None:
        raise EdgeStreamError(
            -1,
            "telemetry streaming requires the 'edgestreamlog-native' wheel (build it from "
...
class EdgeStreamError(Exception):
    """Raised when a native streaming call fails. ``code`` mirrors ``esl_status``."""

    def __init__(self, code: int, message: Optional[str] = None):
        self.code = code
        super().__init__(f"edgestreamlog error {code}" + (f": {message}" if message else ""))


def _translate(exc: BaseException) -> EdgeStreamError:
    """Convert a native ``edgestreamlog_native.EdgeStreamError`` (args = (code, message)) to ours."""
    args = getattr(exc, "args", ())
    code = args[0] if len(args) >= 1 and isinstance(args[0], int) else -1
    message = args[1] if len(args) >= 2 else None
    return EdgeStreamError(code, message)
...
_NativeError = (_native.EdgeStreamError,) if _native is not None else ()
...
class StreamService:
    ...
    def stream(self, name: str) -> StreamHandle:
        """A handle to the named stream (raises ``EdgeStreamError`` ERR_UNKNOWN_STREAM if absent)."""
        try:
