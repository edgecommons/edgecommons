...
``GGCommons.get_commands()`` and register custom verbs with
...
Lifecycle: constructed and :meth:`CommandInbox.start` started by the ``GGCommons``
...
from ggcommons.uns import Uns, UnsClass, UnsScope
...
class CommandInbox:
    ...
    def __init__(
        ...
        *actions* are injected seams so the built-ins unit-test deterministically;
        the ``GGCommons`` runtime wires the real ones.
    ...
    def register(self, verb: str, handler: CommandHandler) -> None:
        ...
        **Precedence:** no shadowing, ever - registering a :data:`BUILT_IN_VERBS`
        ...
        :raises ggcommons.uns.UnsValidationError: when a verb token violates the
            §2.2 token rule
        """
        if verb is None:
