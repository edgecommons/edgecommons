...
:class:`~ggcommons.config.manager.file_config_manager.FileConfigManager` — it reuses the same
``configuration_changed`` / ``_apply_config`` reload seam, but watches the mount *directory* via
:class:`~ggcommons.utils.directory_watcher.DirectoryWatcher` instead of the file inode.
...
:func:`~ggcommons.parameters.source.is_projection_artifact` (FR-CFG-4).
"""
...
DEFAULT_MOUNT_DIR = "/etc/ggcommons"
