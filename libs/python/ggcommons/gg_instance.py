...
:class:`~ggcommons.uns.Uns` topic builder, (b) the
:class:`~ggcommons.messaging.message_builder.MessageBuilder`, and (c) the app-usable
...
class GgInstance:
    ...
    def __init__(self, instance_id: str, config_manager, include_root: bool,
        ...
        :param messaging_client: the (guarded) messaging handle the facades publish
            ...
            compatibility with existing ``GgInstance(...)`` call sites/tests)
    ...
    def _require_messaging_client(self):
        if self._messaging_client is None:
            raise RuntimeError(
                "this GgInstance was constructed without a messaging_client - the"
