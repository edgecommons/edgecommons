...
- :class:`~ggcommons.facades.data_facade.DataFacade` -- the ``data`` class (the
...
- :class:`~ggcommons.facades.events_facade.EventsFacade` -- the ``evt`` class (operator
  events & alarms): derives the ``evt/{severity}/{type}`` channel from the body.
- :class:`~ggcommons.facades.app_facade.AppFacade` -- the ``app`` class (free-form
...
the Java canonical ``com.mbreissi.ggcommons.facades`` package.
"""
