...
Routing: :attr:`~ggcommons.facades.channel.Channel.LOCAL` (default) or
:attr:`~ggcommons.facades.channel.Channel.NORTHBOUND`; a ``stream`` route is
**rejected** (same reasoning as ``events()``).

Library-internal: obtain via ``gg.instance(id).app()`` or the ``main`` convenience
``gg.app()``.

Mirrors Java's ``AppFacade`` (``com.mbreissi.ggcommons.facades.AppFacade``).
"""
import logging
from typing import Any, Dict, Optional

from ggcommons.facades.channel import Channel
from ggcommons.facades.util import sanitize_channel_path
