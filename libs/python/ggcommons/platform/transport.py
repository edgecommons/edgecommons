...
Defaults from the resolved :class:`~ggcommons.platform.platform.Platform`
(GREENGRASS -> IPC, HOST -> MQTT) and is independently overridable, but
constrained: :attr:`IPC` is valid only on
:attr:`~ggcommons.platform.platform.Platform.GREENGRASS` (the Nucleus provides
the IPC socket). See :func:`~ggcommons.platform.resolver.validate`.
"""
