...
class ConfigManagerTest {
    ...
    void testLoggingConfiguration() throws IOException {
        String configJson = """
                {
                  "logging": {"level": "DEBUG", "java_format": "%d{yyyy-MM-dd HH:mm:ss} [%level] %logger{36} - %msg%n", "fileLogging": {"enabled": true, "filePath": "/var/log/{ComponentName}.log"}, "loggers": {"com.mbreissi": "INFO", "org.apache.http": "WARN"}, "globalControl": true},
                  "metricEmission": {"target": "log"},
                  "heartbeat": {"intervalSecs": 30},
                  "tags": {},
                  "component": {"global": {}}
        ...
        runWithTempConfig(configJson, configManager -> {
            ...
            assertEquals("INFO", loggingConfig.getLoggerLevels().get("com.mbreissi").toString());
