...
import com.mbreissi.edgecommons.utils.Utils;
...
class EdgeCommonsTest
{
    private static EdgeCommons edgeCommons;
    ...
    static void setUpClass()
    {
        ...
        edgeCommons = EdgeCommonsBuilder.create("com.mbreissi.edgecommons.IntegrationTests")
                .withArgs(args)
                .build();
        configService = edgeCommons.getConfigManager();
        messagingService = edgeCommons.getMessaging();
        metricService = edgeCommons.getMetrics();
        LOGGER = LogManager.getLogger(EdgeCommonsTest.class);
    }
...
//
    ...
    void publishMinimalHeaderIpcMessage()
    {
        ...
        header.addProperty("reply_to", "edgecommons/reply");
        message.add("header", header);
        messagingService.publishRaw(topic, message);
        Utils.sleep(200);
        assertNotNull(receivedMessage);
        assertNotNull(receivedMessage.getHeader());
        assertEquals("edgecommons/reply", receivedMessage.getHeader().getReplyTo());
