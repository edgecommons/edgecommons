...
public class ConfigProviderBuilder {
    ...
    public static ConfigProvider build(ConfigManager configManager, String componentName, String thingName, String[] configArgs, com.mbreissi.ggcommons.messaging.MessagingClient messagingClient) {
        ConfigProvider configProvider = switch (configArgs[0].toUpperCase()) {
            ...
            case "CONFIGMAP" -> {
                LOGGER.debug("Using ConfigMap com.mbreissi.ggcommons.config provider");
            ...
            case "ENV" -> {
                LOGGER.debug("Using Environment com.mbreissi.ggcommons.config provider");
                String envVarName = configArgs.length > 1 ? configArgs[1] : "CONFIG";
                yield new EnvironmentConfigProvider(configManager, envVarName);
            }
            case "SHADOW" -> {
                LOGGER.debug("Using Shadow com.mbreissi.ggcommons.config provider");
            ...
            case "GG_CONFIG" -> {
                LOGGER.debug("Using Greengrass com.mbreissi.ggcommons.config provider");
            ...
            case "CONFIG_COMPONENT" -> {
                LOGGER.debug("Using Config com.mbreissi.ggcommons.config provider");
                if (messagingClient == null) {
