 ...
 * reply). Obtain the facade via {@code GGCommons.getCommands()} and register custom verbs with
 ...
 * </ul>
 *
 * <p>Lifecycle: constructed and {@link #start() started} by the {@code GGCommons} runtime after
...
public final class CommandInbox implements AutoCloseable {
     ...
     * injected seams so the built-ins unit-test deterministically; the {@code GGCommons} runtime
     * wires the real ones.
     *
     ...
     * @throws com.mbreissi.ggcommons.uns.UnsValidationException when a verb token violates the
     *                                                           §2.2 token rule
     */
