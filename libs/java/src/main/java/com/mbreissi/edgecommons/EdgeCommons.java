...
import com.mbreissi.edgecommons.health.HealthServer;
...
import com.mbreissi.edgecommons.metrics.MetricEmitterBuilder;
import com.mbreissi.edgecommons.streaming.StreamMetricsBridge;
import com.mbreissi.edgecommons.streaming.StreamService;
import com.mbreissi.edgecommons.credentials.CredentialMetricsBridge;
import com.mbreissi.edgecommons.credentials.CredentialService;
import com.mbreissi.edgecommons.credentials.Credentials;
import com.mbreissi.edgecommons.credentials.SecretRefs;
import com.mbreissi.edgecommons.parameters.DefaultParameterService;
import com.mbreissi.edgecommons.parameters.ParameterService;
import com.mbreissi.edgecommons.parameters.Parameters;
import com.mbreissi.edgecommons.platform.Platform;
import com.mbreissi.edgecommons.platform.PlatformResolver;
import com.mbreissi.edgecommons.platform.ResolvedProfile;
...
{
    private static final Logger LOGGER = LogManager.getLogger(EdgeCommons.class);
     ...
     * returns the same {@link EdgeCommonsInstance} for the same id.
     */
    ...
    /**
     * Constructs a new EdgeCommons instance with the given component name and command line arguments.
     * 
     * @param componentName The name of the Greengrass component
     * @param args Command line arguments passed to the component
     * @deprecated Use {@link EdgeCommonsBuilder#create(String)} instead
     */
    @Deprecated
    ...
    /**
     * Constructs a new EdgeCommons instance with custom application options.
     * 
     * @param componentName The name of the Greengrass component
     * @param args Command line arguments passed to the component
    ...
    /**
     * Constructs a new EdgeCommons instance with custom options and message reception settings.
     * 
     * @param componentName The name of the Greengrass component
     * @param args Command line arguments passed to the component
     * @param appOptions Custom options for the application
     ...
     */
    protected EdgeCommons() {
        // Empty constructor for testing
    }
    
    /**
     * Initializes the EdgeCommons instance with the specified parameters.
    ...
    void init(String componentName, String[] args, Options appOptions, boolean receiveOwnMessages)
    {
        try {
            ...
            LOGGER.error("Failed to initialize EdgeCommons: {}", e.getMessage(), e);
            throw new RuntimeException("Failed to initialize EdgeCommons: " + e.getMessage(), e);
        }
    }
     ...
     * a {@link EdgeCommonsInstance} whose {@code uns()} mints topics with — and whose
     ...
     * @throws com.mbreissi.edgecommons.uns.UnsValidationException when the token violates the
     *                                                           §2.2 token rule
     * @throws IllegalStateException when called before initialization completes
     */
    public EdgeCommonsInstance instance(String instanceId)
    {
        ...
        return instanceHandles.computeIfAbsent(instanceId, id -> {
            ...
            return new EdgeCommonsInstance(id, cm, cm.isTopicIncludeRoot(), messagingClient, streamSink(),
                    clock);
        });
    }
    ...
    private ConfigManager requireResolvedIdentity()
    {
        ...
        {
            throw new IllegalStateException("EdgeCommons is not initialized: the component"
    ...
    {
        shutdownHook = new Thread(this::onShutdownSignal, "edgecommons-shutdown");
        try
        {
    ...
    {
        LOGGER.info("Received termination signal; shutting down EdgeCommons gracefully");
        shuttingDown = true;
        shutdownInternal(false);
    }

    /**
     * Shuts down this EdgeCommons instance, releasing background timers, threads and connections
