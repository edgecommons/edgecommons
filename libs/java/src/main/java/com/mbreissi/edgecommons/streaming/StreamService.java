...
/**
 * Java handle to the native {@code edgestreamlog} {@code StreamService}: opens/recovers every stream
...
public final class StreamService implements AutoCloseable {
    ...
    private final EdgeStreamNative n;
    private volatile MemorySegment service; // null after close
    private final String configJson;

    private StreamService(EdgeStreamNative n, MemorySegment service, String configJson) {
        this.n = n;
     ...
     *
     * @throws EdgeStreamException if the config is invalid or a stream fails to open
     */
    public static StreamService open(String configJson) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            if (rc != EdgeStreamException.OK) {
                throw new EdgeStreamException(rc, n.takeError(err));
            }
     ...
     *
     * @throws EdgeStreamException with {@link EdgeStreamException#ERR_UNKNOWN_STREAM} if not configured
     */
    ...
    public StreamStats stats(String name) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            if (rc != EdgeStreamException.OK) {
                throw new EdgeStreamException(rc, null);
            }
    ...
    /**
     * Whether the native {@code edgestreamlog} library can be loaded (so durable streaming is
