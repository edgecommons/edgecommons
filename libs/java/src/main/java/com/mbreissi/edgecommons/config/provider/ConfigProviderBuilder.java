...
public class ConfigProviderBuilder {
    ...
    public static ConfigProvider build(ConfigManager configManager, String componentName, String thingName, String[] configArgs, com.mbreissi.edgecommons.messaging.MessagingClient messagingClient) {
        ConfigProvider configProvider = switch (configArgs[0].toUpperCase()) {
            ...
            case "CONFIGMAP" -> {
                LOGGER.debug("Using ConfigMap com.mbreissi.edgecommons.config provider");
                // -c CONFIGMAP [mountDir] [key]; defaults applied inside the provider
                // (/etc/edgecommons, config.json). The k8s-native source; default on KUBERNETES.
            ...
            case "ENV" -> {
                LOGGER.debug("Using Environment com.mbreissi.edgecommons.config provider");
                String envVarName = configArgs.length > 1 ? configArgs[1] : "CONFIG";
                yield new EnvironmentConfigProvider(configManager, envVarName);
            }
            case "SHADOW" -> {
                LOGGER.debug("Using Shadow com.mbreissi.edgecommons.config provider");
            ...
            case "GG_CONFIG" -> {
                LOGGER.debug("Using Greengrass com.mbreissi.edgecommons.config provider");
            ...
            case "CONFIG_COMPONENT" -> {
                LOGGER.debug("Using Config com.mbreissi.edgecommons.config provider");
                if (messagingClient == null) {
