...
/**
 * Forwards the native {@code edgestreamlog} core's log events into log4j2, so streaming logs land in
 * the component's normal log stream instead of being dropped. Installed once when the native library
 * is loaded (see {@link EdgeStreamNative}); registers a Panama upcall as the C {@code esl_log_cb}.
 */
final class NativeLogBridge {
    ...
    static synchronized void install(EdgeStreamNative n) {
        if (installed) {
            return;
        }
        try {
            ...
            SELF.warn("could not install edgestreamlog log bridge; core logs will not be forwarded", t);
        }
    }

    /**
     * The C {@code esl_log_cb}. May be invoked from native background threads; never throws back
    ...
    static void onLog(MemorySegment userData, int level, MemorySegment target, MemorySegment message) {
        try {
            String t = readCString(target, "edgestreamlog");
