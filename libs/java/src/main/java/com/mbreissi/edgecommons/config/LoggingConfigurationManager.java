...
public class LoggingConfigurationManager {
    ...
    private static final String EDGECOMMONS_LOGGER_PREFIX = "com.mbreissi.edgecommons";
     ...
     * Only configures loggers under the EdgeCommons namespace.
     */
    public void configureLogging() {
        try {
            LoggingConfiguration loggingConfig = configManager.getLoggingConfig();
            
            // Only configure EdgeCommons-specific loggers to avoid conflicts
            configureEdgeCommonsLoggers(loggingConfig);
            
        } catch (Exception e) {
            // Fallback to system logging if configuration fails
            System.err.println("Failed to configure EdgeCommons logging: " + e.getMessage());
        }
    }
    
    private void configureEdgeCommonsLoggers(LoggingConfiguration loggingConfig) {
        LoggerContext context = (LoggerContext) LogManager.getContext(false);
        Configuration config = context.getConfiguration();
        
        // Configure the main EdgeCommons logger
        org.apache.logging.log4j.core.config.LoggerConfig loggerConfig = 
            config.getLoggerConfig(EDGECOMMONS_LOGGER_PREFIX);
        
        if (loggerConfig.getName().equals(EDGECOMMONS_LOGGER_PREFIX)) {
            // Logger already exists, update its level
            loggerConfig.setLevel(loggingConfig.getLevel());
        } else {
            // Create new logger configuration for EdgeCommons
            ...
            config.addLogger(EDGECOMMONS_LOGGER_PREFIX, newLoggerConfig);
        }
        ...
        for (Map.Entry<String, Level> entry : loggerLevels.entrySet()) {
            ...
            // Only configure loggers under EdgeCommons namespace to avoid conflicts
            if (loggerName.startsWith(EDGECOMMONS_LOGGER_PREFIX)) {
