...
/**
 * Bridges a {@code callback}-type edgestreamlog stream to a host {@link SinkFunction}. Registers a
 * single process-wide Panama upcall as the C {@code esl_set_sink_callback}; the export engine
 ...
 * {@link SinkOutcome} back into the core-owned {@code EslSinkOutcome} out-struct.
 *
...
final class SinkBridge {
    ...
    // ----- C-ABI struct layouts (must match edgestreamlog.h / ffi.rs) -----

    // struct esl_sink_record_t { u64 offset; u64 ts_ms; const u8* pk; usize pk_len;
    ...
    // struct esl_sink_outcome_t { c_int status; u64* failed_offsets; usize failed_cap;
    ...
    // ESL_SINK_* status codes (ffi.rs).
    private static final int ESL_OK = 0;
    ...
    /**
     * The C {@code esl_sink_cb}: {@code (user_data, records, n, *outcome) -> int}. Invoked on the
    ...
    /** Write the {@link SinkOutcome} into the core-owned {@code EslSinkOutcome} out-struct. */
