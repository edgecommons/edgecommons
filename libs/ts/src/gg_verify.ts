 ...
 *
 * Runs as a deployed component built through the FULL `GGCommonsBuilder` (so the
...
import { GGCommons, GGCommonsBuilder } from "./ggcommons";
import { MessageBuilder } from "./message";
import { Qos } from "./messaging/types";

const RESULT_PATH = process.env.GGC_TS_VERIFY_OUT ?? "/tmp/ts_gg_verify_result.json";
const METRIC_LOG = "/tmp/ts_gg_metric.log";
...
async function main(): Promise<void> {
  ...
  try {
    gg = await new GGCommonsBuilder("com.ggcommons.TsGgVerify")
    ...
    const rrTopic = `ggcommons/${thing}/tsgg/rr/${rid()}`;
    const rrToken = rid();
    try {
    ...
    // --- raw over IPC ---
    const rawTopic = `ggcommons/${thing}/tsgg/raw/${rid()}`;
    const rawToken = rid();
