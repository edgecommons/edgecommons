/**
 * Unit tests for the EdgeCommons lifecycle (`src/edgecommons.ts`) that don't need a live
 * broker or the native streaming addon.
 *
 * The existing `edgecommons_integration.test.ts` covers the happy STANDALONE+FILE path
...
/** Build a EdgeCommons with the given config object (written nowhere — we mock ENV source). */
async function buildWith(
  cfg: Record<string, unknown>,
  extraArgs: string[] = ["--platform", "HOST", "--transport", "MQTT", "messaging.json"],
): Promise<EdgeCommons> {
  process.env.EDGECOMMONS_LIFECYCLE_CFG = JSON.stringify(cfg);
  const gg = await new EdgeCommonsBuilder("com.example.Lc")
    .args([...extraArgs, "-c", "ENV", "EDGECOMMONS_LIFECYCLE_CFG", "-t", "lc-thing"])
    .build();
  delete process.env.EDGECOMMONS_LIFECYCLE_CFG;
  return gg;
}

describe("EdgeCommons lifecycle (mocked)", () => {
  ...
  it("MQTT transport without a messaging-config path throws a messaging EdgeCommonsError at build", async () => {
    ...
    try {
      let err: unknown;
      try {
        await new EdgeCommonsBuilder("com.example.Lc")
          .args(["--platform", "HOST", "--transport", "MQTT", "-c", "ENV", "EDGECOMMONS_NO_MC", "-t", "lc-thing"])
          .build();
        throw new Error("expected build to throw");
      } catch (e) {
        err = e;
      }
      expect(err).toBeInstanceOf(EdgeCommonsError);
      expect((err as EdgeCommonsError).kind).toBe("Messaging");
      expect((err as EdgeCommonsError).message).toContain("MQTT transport requires a messaging-config path");
      ...
      delete process.env.EDGECOMMONS_NO_MC;
    }
  });

  it("IPC: initializes IPC messaging (no messaging-config file) honoring receiveOwnMessages", async () => {
    process.env.EDGECOMMONS_GG_CFG = JSON.stringify(BASE);
    const built = await new EdgeCommonsBuilder("com.example.Lc")
      .args(["--platform", "GREENGRASS", "-c", "ENV", "EDGECOMMONS_GG_CFG", "-t", "gg-thing"])
      .receiveOwnMessages(true)
      .build();
    delete process.env.EDGECOMMONS_GG_CFG;
    try {
  ...
  it("IPC: receiveOwnMessages defaults to true (Java-canonical convergence)", async () => {
    process.env.EDGECOMMONS_GG_CFG_DEF = JSON.stringify(BASE);
    const built = await new EdgeCommonsBuilder("com.example.Lc")
      .args(["--platform", "GREENGRASS", "-c", "ENV", "EDGECOMMONS_GG_CFG_DEF", "-t", "gg-thing"])
      .build();
    delete process.env.EDGECOMMONS_GG_CFG_DEF;
    try {
      expect(ipcConnect).toHaveBeenCalledWith({ receiveOwnMessages: true });
    } finally {
      await built.close();
  ...
  it("IPC: defaults thing name from AWS_IOT_THING_NAME / NOT_GREENGRASS when -t absent", async () => {
    process.env.EDGECOMMONS_GG_CFG2 = JSON.stringify(BASE);
    delete process.env.AWS_IOT_THING_NAME;
    const built = await new EdgeCommonsBuilder("com.example.Lc")
      .args(["--platform", "GREENGRASS", "-c", "ENV", "EDGECOMMONS_GG_CFG2"])
      .build();
    delete process.env.EDGECOMMONS_GG_CFG2;
    try {
  ...
  it("messaging() throws EdgeCommonsError when no messaging service is wired", async () => {
    ...
    // constructing EdgeCommons with an undefined messaging service via build then patching.
    const gg = await buildWith(BASE);
    try {
      ...
      expect(() => gg.messaging()).toThrow(EdgeCommonsError);
      try {
        gg.messaging();
      } catch (e) {
        expect((e as EdgeCommonsError).kind).toBe("Messaging");
      }
    } finally {
      await gg.close();
    }
  ...
  it("build() rejects when config fails schema validation", async () => {
    ...
    ).rejects.toBeInstanceOf(EdgeCommonsError);
  });
