...
describe("EdgeCommons STANDALONE lifecycle (live broker)", () => {
  let up = false;
  beforeAll(async () => {
    up = await brokerReachable();
  });

  async function build(configPath: string): Promise<EdgeCommons> {
    ...
    return new EdgeCommonsBuilder("com.example.It")
      .args([
        "--platform",
        "HOST",
        "--transport",
  ...
  it("a reload that fails validation keeps the previous snapshot; a throwing listener is caught", async (ctx) => {
    ...
    try {
      gg.addConfigChangeListener({
        // Async rejection: this is the path edgecommons guards with `.catch(...)`.
