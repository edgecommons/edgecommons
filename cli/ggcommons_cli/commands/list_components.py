...
``--source`` / ``$GGCOMMONS_REGISTRY_URL`` (a URL — e.g. if you make the registry public — or a local
...
class ListComponents(CommandBase):
            ...
            "parameters": [
                {
                    ...
                    "description": (
                        ...
                        "edgecommons/registry catalog via gh, or $GGCOMMONS_REGISTRY_URL)"
                    ),
                    "type": "string",
                },
                {
    ...
    def execute_command(self, args: Dict[str, Any]):
        source = args.get("source") or os.environ.get("GGCOMMONS_REGISTRY_URL")
