...
MANIFEST_NAME = "edgecommons-template.json"

# Monorepo root, resolved relative to this file (cli/edgecommons_cli/commands -> repo root).
...
def _bundled_templates_root():
    ...
    try:
        ...
        path = os.path.join(str(files("edgecommons_cli")), "templates")
...
# Languages whose components depend on the edgecommons library via a path dependency
# (Cargo path dep for Rust, npm `file:` dep for TypeScript). These require a valid
# --edgecommons-path and a language-specific default (libs/<lang> in this monorepo).
_EDGECOMMONS_PATH_DEFAULTS = {
    "RUST": ("libs", "rust"),
    "TYPESCRIPT": ("libs", "ts"),
}
...
# Where a generated component resolves the edgecommons library from. `local` = a path/file
...
# `edgecommons upgrade --to <ver>` bumps a generated component later.
_EDGECOMMONS_VERSION = "0.1.0"
_EDGECOMMONS_GIT_URL = "https://github.com/edgecommons/edgecommons"
...
class CreateComponent(CommandBase):
    ...
    @classmethod
    def _default_edgecommons_path(cls, language: Optional[str]) -> str:
        """Default edgecommons library path for a path-dependency language: libs/<lang>
        ...
        parts = _EDGECOMMONS_PATH_DEFAULTS.get(language or "", ("libs", "rust"))
    ...
    def get_json_configuration(cls):
        ...
        # default is re-resolved per language in execute_command (see _default_edgecommons_path).
        default_edgecommons_path = cls._default_edgecommons_path(None)
        return {
            "name": "create-component",
            "description": "Create a new component that uses edgecommons",
            "parameters": [
                {
                    "name": "name",
                    "short": "n",
                ...
                {
                    "name": "edgecommons-path",
                    "short": "g",
                    "description": "Absolute path to the edgecommons library (RUST/TYPESCRIPT "
                                   "only; becomes the Cargo / npm `file:` path dependency)",
                    "type": "string",
                    "required": False,
                    "default": default_edgecommons_path
                },
                {
                    "name": "template-url",
                    "short": "u",
                ...
                {
                    "name": "dep-source",
                    "description": "How the generated component depends on the edgecommons library: "
    ...
    def execute_command(self, args: Dict[str, Any]):
        ...
        # argparse maps --edgecommons-path / --template-url to underscore dest names.
        self.edgecommons_path = args.get('edgecommons_path')
        ...
        static_default = self._default_edgecommons_path(None)
        if self.edgecommons_path == static_default and self.component_language in _EDGECOMMONS_PATH_DEFAULTS:
            self.edgecommons_path = self._default_edgecommons_path(self.component_language)
    ...
    def _placeholder_values(self) -> Dict[str, str]:
        ...
        return {
            ...
            "EDGECOMMONS_PATH": self.edgecommons_path or "",
            "EDGECOMMONS_DEP": self._edgecommons_dep(),
            # The edgecommons release version a generated component depends on (Java pom; also the
            # registry coordinate Rust/TS use via EDGECOMMONS_DEP). `edgecommons upgrade` bumps it later.
            "EDGECOMMONS_VERSION": _EDGECOMMONS_VERSION,
        }

    def _edgecommons_dep(self) -> str:
        """The dependency-declaration fragment a path-dep template (Rust/TS) substitutes for
        ``<<EDGECOMMONS_DEP>>``, chosen by --dep-source. `local` = a path/file dependency on the
        ...
        if self.dep_source == "registry":
            if lang == "RUST":
                return f'git = "{_EDGECOMMONS_GIT_URL}", tag = "rust-lib/v{_EDGECOMMONS_VERSION}"'
            if lang == "TYPESCRIPT":
                return f"^{_EDGECOMMONS_VERSION}"
            return ""
        path = (self.edgecommons_path or "").replace("\\", "/")
        if lang == "RUST":
            return f'path = "{path}"'
        if lang == "TYPESCRIPT":
            return f"file:{path}"
    ...
    def _run_wizard(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        args['dep_source'] = self._prompt_choice(
            "edgecommons dependency source", list(VALID_DEP_SOURCES),
            (args.get('dep_source') or _DEFAULT_DEP_SOURCE))
        if args['dep_source'] == "local" and args['language'] in _EDGECOMMONS_PATH_DEFAULTS:
            default_path = self._default_edgecommons_path(args['language'])
            args['edgecommons_path'] = self._prompt(
                "Path to the local edgecommons library", args.get('edgecommons_path') or default_path)
    ...
    def _validate_args(self):
        ...
        if self.dep_source == "local" and self.component_language in _EDGECOMMONS_PATH_DEFAULTS:
            if not self.edgecommons_path or not os.path.isdir(self.edgecommons_path):
                raise ValueError(
                    f"{self.component_language} components need a valid edgecommons library "
                    f"path, but '{self.edgecommons_path}' does not exist. "
                    f"Pass --edgecommons-path <abs path>."
                )
            # Normalize to forward slashes (Cargo/npm `file:` deps accept them on all OSes).
            self.edgecommons_path = self.edgecommons_path.replace("\\", "/")
    ...
    def _apply_manifest(self, manifest: dict, target_dir: str, values: Dict[str, str]):
        """Apply a template manifest. Schema:
            {
              "required":      ["EDGECOMMONS_PATH", ...],       # placeholders that must be non-empty
    ...
    def _lint_recipe(self, recipe_path: str):
        ...
        if not os.path.isfile(recipe_path):
            return
        from edgecommons_cli.recipe_lint import lint_recipe_file
