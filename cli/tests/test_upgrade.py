from edgecommons_cli.commands.upgrade import Upgrade


def test_bump_pom(tmp_path):
    pom = tmp_path / "pom.xml"
    pom.write_text(
        "<dependency><groupId>com.mbreissi.edgecommons</groupId>"
        "<artifactId>edgecommons</artifactId><version>1.1.9-SNAPSHOT</version></dependency>"
    )
...
def test_bump_requirements(tmp_path):
    ...
    assert "edgecommons==2.0.0" in req.read_text()
...
def test_bump_cargo_version_dep(tmp_path):
    ...
    assert 'edgecommons = "0.2"' in cargo.read_text()


def test_bump_cargo_path_dep_is_noop(tmp_path):
    cargo = tmp_path / "Cargo.toml"
    original = '[dependencies]\nedgecommons = { path = "../core-rust-lib" }\n'
...
def test_execute_with_no_dependency(tmp_path, capsys):
    ...
    assert "No edgecommons dependency" in capsys.readouterr().out
