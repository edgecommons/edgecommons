...
# Templates live as siblings of the CLI repo in the edgecommons workspace.
...
def test_generate_component(language, tmp_path):
    ...
    if language == "RUST" and not GG_LIB.is_dir():
        pytest.skip("edgecommons-rust-lib not present")
    ...
    assert not (project / "edgecommons-template.json").exists()
...
def test_rust_requires_valid_edgecommons_path(tmp_path):
    template = TEMPLATES["RUST"]
    if not template.is_dir():
        pytest.skip("rust template not present")
    args = _args("RUST", tmp_path, template)
    args["edgecommons_path"] = str(tmp_path / "does-not-exist")
