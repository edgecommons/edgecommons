# EdgeCommons test-infra

Shared integration-test infrastructure for the EdgeCommons libraries
...
Container name: `edgecommons-emqx`.
...
Integration tests locate the certs via the **`EDGECOMMONS_TLS_CERTS_DIR`** environment
variable (absolute path to this repo's `tls-certs/`). Example (Python):
```bash
EDGECOMMONS_TLS_CERTS_DIR=/abs/path/to/test-infra/tls-certs \
...
Or per library (broker up + `EDGECOMMONS_TLS_CERTS_DIR` exported first):
```bash
# Python — locates certs via EDGECOMMONS_TLS_CERTS_DIR
EDGECOMMONS_TLS_CERTS_DIR=$PWD/tls-certs \
...
# Rust — gated on EDGECOMMONS_IT_MQTT=1 + per-file cert vars
EDGECOMMONS_IT_MQTT=1 EDGECOMMONS_IT_MQTT_CA=$PWD/tls-certs/ca.crt \
  EDGECOMMONS_IT_MQTT_CERT=$PWD/tls-certs/client.crt \
  EDGECOMMONS_IT_MQTT_KEY=$PWD/tls-certs/client.key \
...
each deployed edgecommons component:
...
EDGECOMMONS_IT_GG=1 python3 test_deployed_component.py
# or via pytest (skips unless EDGECOMMONS_IT_GG=1):
EDGECOMMONS_IT_GG=1 python3 -m pytest test_deployed_component.py -v
# Override the component set (default = the four skeleton examples):
EDGECOMMONS_IT_COMPONENTS="com.a,com.b" EDGECOMMONS_IT_GG=1 python3 test_deployed_component.py
```
