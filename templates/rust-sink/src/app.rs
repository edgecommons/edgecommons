...
use crate::dest::DestinationConfig;
...
pub(crate) fn key_for(sink_id: &str, topic: &str, msg: &Message) -> String {
...
pub(crate) fn apply_defaults(sink: &mut SinkConfig, defaults: &serde_json::Value) {
...
mod tests {
    ...
    #[test]
    fn defaults_fill_only_what_a_sink_left_untouched() {
        // A sink that set neither knob inherits both maxQueue and the whole retry block.
        let mut inherit = sink_config("archive");
        apply_defaults(
            &mut inherit,
            &json!({ "maxQueue": 64, "retry": { "baseDelayMs": 250, "maxDelayMs": 5000, "giveUpAfterMs": 30000 } }),
        );
        assert_eq!(inherit.max_queue, 64);
        assert_eq!(inherit.retry.base_delay_ms, 250, "the whole retry block was inherited");
        assert_eq!(inherit.retry.give_up_after_ms, 30_000);

        // A sink that set its own retry keeps it — an all-or-nothing block, never a partial default.
        let mut explicit: SinkConfig = serde_json::from_value(json!({
            "id": "archive",
            "subscribe": "ecv1/+/+/+/data/#",
            "destination": { "type": "local", "path": "/var/lib/out" },
            "retry": { "baseDelayMs": 999 }
        }))
        .unwrap();
        apply_defaults(&mut explicit, &json!({ "retry": { "baseDelayMs": 250 } }));
        assert_eq!(explicit.retry.base_delay_ms, 999, "an explicit retry block is not overridden");
    }
}
