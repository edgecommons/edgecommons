#!/usr/bin/env bash
# Generate throwaway TLS test certificates for the local-broker (EMQX) secure-connection
# tests: a CA, a server cert for localhost (used by the broker), and a client cert
# (used for mutual TLS). Output goes to tests/tls-certs/. Safe to re-run.
#
# These are TEST-ONLY certs — never use them in production, and they are gitignored.
set -euo pipefail
